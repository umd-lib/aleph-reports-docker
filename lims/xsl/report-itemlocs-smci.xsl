...
>
   ...
   <xsl:key name="material"     match="loc" use="concat(@sublibrary,'--',@material)"/>
   <xsl:key name="collection" match="loc" use="concat(@sublibrary,'--',@material,'--',@collection)"/>
...
-->
               <xsl:value-of select="'Sublibrary/Material/Collection/Item Status Combinations'"/>
             </font>
             <br/>
             <font>
   ...
   <xsl:template name="header">
     <!-- Header -->
       <tr  style="font-family=Arial; font-size=12pt">
         ...
         <td valign="bottom"><strong>Material</strong></td>
   ...
   <xsl:template name="body">
     ...
     <xsl:for-each select="loc[generate-id() =
       ...
       <xsl:for-each select="($inst-nodes)[generate-id() =
         ...
         <!-- Loop through unique sublibrary/material -->
         <xsl:for-each
              select="($sublibrary-nodes)[generate-id() =
              generate-id(key('material', concat($sublibrary,'--',@material))[1])]">

           <xsl:sort select="@material"/>
           <xsl:variable name="material" select="@material"/>
           <xsl:variable name="material-nodes" select="key('material', concat($sublibrary,'--',$material))"/>
         
           <!-- Summarize the material -->
           <xsl:call-template name="material">
             <xsl:with-param name="sublibrary" select="$sublibrary"/>
             <xsl:with-param name="material" select="$material"/>
             <xsl:with-param name="material-nodes" select="$material-nodes"/>
           </xsl:call-template>

           <!-- Loop through unique sublibrary/material/collection -->
           <xsl:for-each
                select="($material-nodes)[generate-id() =
                generate-id(key('collection', concat($sublibrary,'--',$material,'--',@collection))[1])]">
             <xsl:variable name="collection" select="@collection"/>
             <xsl:variable name="collection-nodes"
                           select="key('collection', concat($sublibrary,'--',$material,'--',$collection))"/>
   ...
   <xsl:template name="material">
     ...
     <tr id="summary">
       ...
       <td id="vbar">-</td>
       <td>-</td>
       <td><xsl:value-of select="sum($material-nodes/items)"/></td>

       <td id="vbar"><xsl:value-of select="sum($material-nodes/cur-loans-status-A)"/></td>
       <td><xsl:value-of select="sum($material-nodes/range-loans-status-A)"/></td>
       <td><xsl:value-of select="sum($material-nodes/cur-loans-status-C)"/></td>
       <td><xsl:value-of select="sum($material-nodes/cur-loans-status-L)"/></td>
       <td></td>
     </tr>
