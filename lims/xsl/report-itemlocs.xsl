...
>
   ...
   <xsl:key name="collection" match="loc" use="concat(@sublibrary,'--',@collection)"/>
   ...
   <xsl:template name="body">
     ...
     <xsl:for-each select="loc[generate-id() =
       ...
       <xsl:for-each select="($inst-nodes)[generate-id() =
         ...
         <!-- Loop through unique sublibrary/collection -->
         <xsl:for-each
              select="($sublibrary-nodes)[generate-id() =
              generate-id(key('collection', concat($sublibrary,'--',@collection))[1])]">
         
           <xsl:variable name="collection" select="@collection"/>
           <xsl:variable name="collection-nodes" select="key('collection', concat($sublibrary,'--',$collection))"/>
